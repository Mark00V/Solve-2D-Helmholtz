...
rohair=1.21              # Density
pos_quelle = (7.5, 0.5)  # Position of monopole sound source
Zi=(rohair*c)            # Impedance at boundary (here: left, top, right boundary of )
# Output
spl_low,spl_hi = 60,120  # Crop result between those values
# Mesh
elementindex=np.loadtxt("elementindex_schallwand.csv",delimiter=" ", dtype=str)          # Elements
pointcoordinates=np.loadtxt("pointcoordinates_schallwand.csv",delimiter=" ", dtype=str)  # Nodepositions
# =========================================
...
numintgld1=np.array([[0.21132486,0.78867513],[0.50000000,0.50000000]],dtype=np.single) # Numerische Integration 1D Knoten und Gewichte
numintgld2nodes=np.array([[0.21132487, 0.21132487], [0.21132487, 0.78867513], \
                          [0.78867513, 0.21132487], [0.78867513, 0.78867513]],dtype=np.single)  # Numerische Integration 2D Knoten
numintgld2weights=np.array([0.25000000, 0.25000000, 0.25000000, 0.25000000],dtype=np.single)      # Numerische Integration 2D Gewichte
...
for ie in range(maxelement):
    if ie%10==0:
        print("\rElement: "+str(ie)+"/"+str(maxelement), end='',flush=True)
...
newspldata=datacropped(solutionspl,spl_low,spl_hi)
